...
class AttModel(CaptionModel):
    def __init__(self, opt):
        ...
        self.vis_embed = nn.Sequential(nn.Embedding(self.detect_size+1,
                                       ...
                                       nn.ReLU(),
                                       nn.Dropout(
                                           self.drop_prob_lm, inplace=True)
                                       )
        ...
        if self.transfer_mode in ('cls', 'both'):
            ...
            vis_classifiers = opt.glove_clss.new(
                self.detect_size+1, cls_score_w.size(1)).fill_(0)
        ...
        if opt.enable_visdom:
            ...
            self.vis = visdom.Visdom(
                server=opt.visdom_server, env='vis-'+opt.id)
    ...
    def forward(self, segs_feat, seq, gt_seq, num, ppls, gt_boxes, mask_boxes, ppls_feat, frm_mask, sample_idx, pnt_mask, opt, eval_opt={}):
        if opt == 'MLE':
            ...
            seq, seqLogprobs, att2, sim_mat = self._sample(
                segs_feat, ppls, num, ppls_feat, sample_idx, pnt_mask, eval_opt)
    ...
    def _forward(self, segs_feat, input_seq, gt_seq, ppls, gt_boxes, mask_boxes, num, ppls_feat, frm_mask, sample_idx, pnt_mask, eval_obj_ground=False):
        ...
        seq = gt_seq[:, :self.seq_per_img, :].clone(
        ).view(-1, gt_seq.size(2))  # choose the first seq_per_img

        seq = torch.cat(
            (Variable(seq.data.new(seq.size(0), 1).fill_(0)), seq), 1)
        ...
        vis_word = Variable(torch.Tensor(
            range(0, self.detect_size+1)).type(input_seq.type()))
        ...
        if self.test_mode:
            ...
            if not eval_obj_ground:
                ...
                sim_mat_masked = torch.masked_select(torch.max(sim_mat_static, dim=1)[
                                                     1].unsqueeze(1).expand_as(sim_target), sim_mask)
        ...
        if self.att_model == 'transformer':  # Masked Transformer does not support box supervision yet
            ...
            return lm_loss.unsqueeze(0), lm_loss.new(1).fill_(0), lm_loss.new(1).fill_(0), \
                lm_loss.new(1).fill_(0), lm_loss.new(
                    1).fill_(0), lm_loss.new(1).fill_(0)
            ...
            decoded = F.log_softmax(
                self.beta * self.logit(rnn_output), dim=2)  # text word prob
            ...
            xt_clamp = torch.clamp(
                input_seq[:, 1:seq_cnt+1, 0].clone()-self.vocab_size, min=0)
            ...
            if not eval_obj_ground:
                ...
                ground_weights = self._grounder(
                    xt_all, g_pool_feats, pnt_mask[:, 1:], bias+att2_weights)
    ...
    def _sample(self, segs_feat, ppls, num, ppls_feat, sample_idx, pnt_mask, opt={}):
        ...
        # visual words embedding
        vis_word = Variable(torch.Tensor(
            range(0, self.detect_size+1)).type(fc_feats.type())).long()
        vis_word_embed = self.vis_embed(vis_word)
        assert(vis_word_embed.size(0) == self.detect_size+1)
        ...
        if self.att_model == 'transformer':
            if self.att_input_mode == 'both':
                seq = self.cap_model(
                    [conv_feats, pool_feats], [], infer=True, seq_length=self.seq_length)
            elif self.att_input_mode == 'featmap':
                seq = self.cap_model(
                    [conv_feats, conv_feats], [], infer=True, seq_length=self.seq_length)
            elif self.att_input_mode == 'region':
                seq = self.cap_model(
                    [pool_feats, pool_feats], [], infer=True, seq_length=self.seq_length)
            ...
            for t in range(self.seq_length + 1):
                if t == 0:  # input <bos>
                    ...
                    sampleLogprobs_tmp, it_tmp = torch.topk(
                        logprobs.data, 2, dim=1)
                    # mask on non-unk
                    unk_mask = (it_tmp[:, 0] != self.unk_idx)
                    sampleLogprobs = unk_mask.float(
                    )*sampleLogprobs_tmp[:, 0] + (1-unk_mask.float())*sampleLogprobs_tmp[:, 1]
                    it = unk_mask.long()*it_tmp[:, 0] + \
                        (1-unk_mask.long())*it_tmp[:, 1]
                    it = it.view(-1).long()
                else:
                    if temperature == 1.0:
                ...
                if t < self.seq_length:
                    ...
                    decoded = F.log_softmax(
                        self.beta * self.logit(rnn_output), dim=1)
    ...
    def _sample_beam(self, segs_feat, ppls, num, ppls_feat, sample_idx, pnt_mask, opt={}):
        ...
        vis_offset = (torch.arange(0, beam_size) *
                      rois_num).view(beam_size).type_as(ppls.data).long()
        roi_offset = (torch.arange(0, beam_size)*(rois_num+1)
                      ).view(beam_size).type_as(ppls.data).long()
        ...
        for k in range(batch_size):
            ...
            beam_pool_feats = pool_feats[k:k+1].expand(
                beam_size, rois_num, self.rnn_size).contiguous()
            if self.att_input_mode in ('both', 'featmap'):
                beam_conv_feats = conv_feats[k:k+1].expand(
                    beam_size, conv_feats.size(1), self.rnn_size).contiguous()
                beam_p_conv_feats = p_conv_feats[k:k+1].expand(
                    beam_size, conv_feats.size(1), self.att_hid_size).contiguous()
            else:
                beam_conv_feats = beam_pool_feats.new(1, 1).fill_(0)
                beam_p_conv_feats = beam_pool_feats.new(1, 1).fill_(0)
            beam_p_pool_feats = p_pool_feats[k:k+1].expand(
                beam_size, rois_num, self.att_hid_size).contiguous()

            beam_ppls = ppls[k:k+1].expand(beam_size, rois_num, 7).contiguous()
            beam_pnt_mask = pnt_mask[k:k +
                                     1].expand(beam_size, rois_num+1).contiguous()

            it = fc_feats.data.new(beam_size).long().zero_()
            xt = self.embed(Variable(it))

            beam_sim_mat_static_update = sim_mat_static_update[k:k+1].expand(
                beam_size, self.detect_size+1, rois_num)
            ...
            seq[:, k] = self.done_beams[k][0]['seq'].cuda()
