        ...
        if hasattr(self, 'alpha_net'):
            ...
            if self.alpha_net.weight.size(1) == self.att_hid_size:
                if self.region_attn_mode == 'mix_mul':
                    # element-wise multiplication attn.
                    dot = att * att_h.unsqueeze(1)
                else:
                    # batch * att_size * att_hid_size
                    dot = att + att_h.unsqueeze(1)
            else:
            ...
            hAflat = torch.matmul(att, att_h.view(
                batch_size, self.att_hid_size, 1))
...
class TopDownCore(nn.Module):
    def __init__(self, opt, use_maxout=False):
        ...
        if self.att_input_mode == 'dual_region':
            ...
            self.dual_pointer = nn.Sequential(
                nn.Linear(opt.rnn_size, 1), nn.Sigmoid())
    ...
    def forward(self, xt, fc_feats, conv_feats, p_conv_feats, pool_feats, p_pool_feats, att_mask, pnt_mask, state, sim_mat_static_update):
        ...
        att2, att2_weight, att_h = self.attention2(
            h_att, pool_feats, p_pool_feats, att_mask[:, 1:], pnt_mask[:, 1:])
