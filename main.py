...
def eval_grounding(opt, vis=None):
    ...
    with open(attn_file, 'w') as f:
        json.dump({'results': att2_output, 'eval_mode': 'GT', 'external_data': {
                  'used': True, 'details': 'Object detector pre-trained on Visual Genome on object detection task.'}}, f)
    grd_file = 'results/grd-gt-sent-results-'+opt.val_split+'-'+opt.id+'.json'
    with open(grd_file, 'w') as f:
        json.dump({'results': grd_output, 'eval_mode': 'GT', 'external_data': {
                  'used': True, 'details': 'Object detector pre-trained on Visual Genome on object detection task.'}}, f)
...
def eval(epoch, opt, vis=None, vis_window=None):
    ...
    if opt.eval_obj_grounding or opt.language_eval:
        for step in range(len(dataloader_val)):
            ...
            input_ppls.resize_(proposals.size()).data.copy_(proposals)
            mask_ppls.resize_(ppl_mask.size()).data.copy_(ppl_mask)
            pnt_mask = torch.cat((mask_ppls.new(mask_ppls.size(0), 1).fill_(
                0), mask_ppls), dim=1)  # pad 1 column from a legacy reason
            ppls_feat.resize_(region_feat.size()).data.copy_(region_feat)
    ...
    *\n*            Please submit your results to the eval server!  *')
    ...
    if opt.eval_obj_grounding:
        ...
        with open(attn_file, 'w') as f:
            json.dump({'results': grd_output, 'eval_mode': 'gen', 'external_data': {
                      'used': True, 'details': 'Object detector pre-trained on Visual Genome on object detection task.'}}, f)
    ...
    if opt.start_from is not None:
        if opt.load_best_score == 1:
            ...
            info_path = os.path.join(
                opt.start_from, 'infos_'+opt.id+'-best.pkl')
        else:
        ...
        # to_load_dict = {
        # k: v for k, v in loaded_state_dict.items() if 'obj_interact' not in k}
        model.load_state_dict(torch.load(model_path))
        # model.load_state_dict(to_load_dict)
